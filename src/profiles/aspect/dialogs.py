...
class AddAspectUpgrade(QDialog):
    def __init__(self, aspect_upgrades: list[str], parent: QWidget | None = None) -> None:
        ...
        catalog = GameCatalog()
        for name in sorted(
            (name for name in catalog.aspect_list if name not in aspect_upgrades), key=catalog.aspect_label
        ):
            self.name_input.addItem(catalog.aspect_label(name), name)
    ...
    def get_value(self) -> str:
        value = self.name_input.currentData()
