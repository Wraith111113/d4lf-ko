...
class ProfileDocumentStore:
    ...
    def save_existing(
        ...
        if save_path.exists() and backup_original:
            ...
            # Keep the immediately previous version as well. The first backup is
            # the imported original; dated backups make later editor saves recoverable.
            timestamp = datetime.datetime.now(tz=datetime.UTC).strftime("%Y%m%d_%H%M%S_%f")
            previous_path = backup_path.parent / f"{save_path.stem}_{timestamp}.yaml"
            shutil.copyfile(save_path, previous_path)
    ...
    def _write_profile(
        ...
        try:
            ProfileModel.model_validate(profile.model_dump())
        except ValidationError as exc:
            raise _profile_validation_error(save_path, exc) from exc
