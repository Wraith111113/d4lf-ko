...
class ProfileEditor(QTabWidget):
    def __init__(self, loaded_profile: LoadedProfile, parent: QWidget | None = None) -> None:
        ...
        # Add tabs with icons
        self.addTab(self.affixes_tab, tr(AFFIXES_TABNAME))
        self.addTab(self.charms_tab, tr(CHARMS_TABNAME))
        self.addTab(self.seals_tab, tr(SEALS_TABNAME))
        self.addTab(self.aspect_upgrades_tab, tr(ASPECT_UPGRADES_TABNAME))
        self.addTab(self.sigils_tab, tr(SIGILS_TABNAME))
        self.addTab(self.tributes_tab, tr(TRIBUTES_TABNAME))
        self.addTab(self.uniques_tab, tr(UNIQUES_TABNAME))
    ...
    def tab_changed(self, index: int) -> None:
        if self.widget(index) is self.affixes_tab:
            self.affixes_tab.load()
        elif self.widget(index) is self.charms_tab:
            self.charms_tab.load()
        elif self.widget(index) is self.seals_tab:
            self.seals_tab.load()
        elif self.widget(index) is self.aspect_upgrades_tab:
            self.aspect_upgrades_tab.load()
        elif self.widget(index) is self.sigils_tab:
            self.sigils_tab.load()
        elif self.widget(index) is self.tributes_tab:
            self.tributes_tab.load()
        elif self.widget(index) is self.uniques_tab:
