...
class AffixesTab(QWidget):
    ...
    def setup_ui(self) -> None:
        ...
        self.item_picker = QComboBox(self)
        self.item_picker.currentIndexChanged.connect(self.tab_widget.setCurrentIndex)
        self.tab_widget.currentChanged.connect(self.item_picker.setCurrentIndex)
        ...
        if not self._editing_disabled:
            for affix_group in self.affixes_model:
                for item_name in affix_group.root:
                    ...
                    self.item_picker.addItem(self._display_name(item_name, group.config.item_type), item_name)

        add_item_button = QPushButton()
        add_item_button.setText(tr("Create Item"))
        add_item_button.clicked.connect(self.add_item_type)

        remove_item_button = QPushButton()
        remove_item_button.setText(tr("Remove Item"))
        remove_item_button.clicked.connect(self.remove_item_type)

        set_all_min_greater_affix_button = QPushButton(tr("Set All Min GAs (Excludes Auto Synced Items)"))
        convert_all_to_min_percent_button = QPushButton(tr("Convert All To Min %"))
        set_all_min_power_button = QPushButton(tr("Set all minPower"))
        ...
        self.main_layout.addWidget(self.item_picker)
        self.main_layout.addWidget(self.tab_widget)
        self.tab_widget.tabBar().hide()

    @staticmethod
    def _display_name(name: str, item_types: list[ItemType]) -> str:
        if name == "Mace" and item_types and all(is_weapon(item) for item in item_types):
            return "무기"
        item_type = ItemType.__members__.get(name)
        if item_type is not None:
            return GameCatalog().item_type_label(item_type)
        if name == "Ring2":
            return f"{GameCatalog().item_type_label(ItemType.Ring)} 2"
        return name
    ...
    def close_tab(self, index: int) -> None:
        ...
        self.item_picker.removeItem(index)
