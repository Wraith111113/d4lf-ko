...
class UniqueAspectWidget(QWidget):
    ...
    def create_aspect_name_combobox(self) -> None:
        ...
        catalog = GameCatalog()
        for aspect_name in sorted(aspects, key=catalog.unique_label):
            self.name_combo.addItem(catalog.unique_label(aspect_name), aspect_name)
        self.name_combo.setMaximumWidth(600)
        if self.unique_aspect.name in GameCatalog().aspect_unique_dict:
            index = self.name_combo.findData(self.unique_aspect.name)
            if index >= 0:
    ...
    def update_name(self, current_text: str | None = None) -> None:
        del current_text
        aspect_name = self.name_combo.currentData()
