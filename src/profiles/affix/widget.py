...
class AffixWidget(QWidget):
    ...
    def populate_affix_combo(self) -> None:
        ...
        if is_seal:
            ...
            for key, label in sorted(self.filtered_affixes.items(), key=operator.itemgetter(1, 0)):
                self.name_combo.addItem(label, key)
            ...
            if curr_set == target_set and self.affix.name in self.filtered_affixes:
                ...
                self.affix.name = self.name_combo.currentData() or ""
        else:
            for key, label in sorted(affix_dict.items(), key=operator.itemgetter(1, 0)):
                self.name_combo.addItem(label, key)
            if self.affix.name in affix_dict:
                index = self.name_combo.findData(self.affix.name)
    ...
    def create_greater_checkbox(self) -> None:
        self.greater_checkbox = QCheckBox(tr("Greater"))
    ...
    def create_mode_combobox(self) -> None:
        ...
        self.mode_combo.addItem(tr("Actual value"), AFFIX_VALUE_MODE)
        self.mode_combo.addItem(tr("Roll %"), AFFIX_PERCENT_MODE)
        self.mode_combo.setToolTip(
            tr(
                "Actual value compares the number shown in-game. Roll % compares its position within the item's min-max range."
            )
        )
        if self.affix.min_percent_of_affix:
            self.mode_combo.setCurrentIndex(self.mode_combo.findData(AFFIX_PERCENT_MODE))
        else:
            self.mode_combo.setCurrentIndex(self.mode_combo.findData(AFFIX_VALUE_MODE))
    ...
    def update_name(self, current_text: str | None = None) -> None:
        ...
        is_seal = self.get_parent_seal_config() is not None
        affix_dict = self.get_affix_dict()
        text = self.name_combo.currentText() if current_text is None else current_text
        choices = self.filtered_affixes if is_seal else affix_dict
        if not choices:
            self.affix.name = ""
            return
        # Item data is the canonical ID, so translated/sorted labels cannot change it.
        index = self.name_combo.currentIndex()
        if index >= 0 and self.name_combo.itemText(index) == text:
            canonical_name = self.name_combo.itemData(index)
            if isinstance(canonical_name, str) and canonical_name in choices:
                self.affix.name = canonical_name
                return
        # Searching must not erase the last valid selection or switch duplicate labels.
        if choices.get(self.affix.name) == text:
            return
        matches = [key for key, label in choices.items() if label == text]
        if len(matches) == 1:
            self.affix.name = matches[0]

    def refresh_value_input(self) -> None:
        if self.mode_combo.currentData() == AFFIX_PERCENT_MODE:
            self.value_edit.setPlaceholderText(tr("0-100"))
            self.value_edit.setValidator(QIntValidator(0, 100, self.value_edit))
            self.value_edit.setToolTip(
                tr(
                    "Roll %: 80 means the value must reach at least 80% of its possible roll range. A missing range cannot pass."
                )
            )
            display_value = "" if self.affix.min_percent_of_affix == 0 else str(self.affix.min_percent_of_affix)
        else:
            self.value_edit.setPlaceholderText(tr("Min value"))
            self.value_edit.setValidator(QDoubleValidator(self.value_edit))
            self.value_edit.setToolTip(
                tr("Enter the number shown in-game: +5.9% uses 5.9; +2703 Life uses 2703. Leave blank for no minimum.")
            )
    ...
    def update_mode(self, current_text: str | None = None) -> None:
        mode = self.mode_combo.currentData() or current_text or self.mode_combo.currentText()
    ...
    def set_min_percent(self, percent: int, convert_mode: bool = False) -> None:
        if convert_mode and self.mode_combo.currentData() != AFFIX_PERCENT_MODE:
            self.mode_combo.setCurrentIndex(self.mode_combo.findData(AFFIX_PERCENT_MODE))
        if self.mode_combo.currentData() != AFFIX_PERCENT_MODE:
            return
