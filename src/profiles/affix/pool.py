...
class AffixPoolWidget(QWidget):
    ...
    def setup_ui(self) -> None:
        ...
        min_count_label = QLabel(tr("Min Count:"))
        ...
        max_count_label = QLabel(tr("Max Count:"))
        ...
        greater_label = QLabel(tr("Greater"))
        ...
        value_label = QLabel(tr("Threshold"))
    ...
    def remove_selected(self, list_widget: QListWidget) -> None:
        if len(list_widget.selectedItems()) >= len(self.pool.count):
            QMessageBox.information(self, tr("Affixes"), tr("Remove the entire affix pool instead of its last option."))
            return
