...
def _move_items(
    ...
    for index, item in enumerate(occupied):
        ...
        if should_move and item_move_count < num_to_move:
            inv.hover_item(item)
            Mouse.click("right")
            item_move_count += 1
        else:
            remaining_unhandled_slots.append(item)

        if item_move_count >= num_to_move:
            remaining_unhandled_slots.extend(occupied[index + 1 :])
            break
