...
def _is_charm_slot_unlock(text: str) -> bool:
    ...
    return (normalized.startswith("unlocks ") and "charm slot" in normalized) or (
        "부적 슬롯" in normalized and ("해제" in normalized or "잠금 해제" in normalized)
    )
...
def _add_sigil_affixes_from_tts(tts_section: list[str], item: Item) -> Item:
    ...
    if item.name in GameCatalog().sigil_name_aliases:
        item.name = GameCatalog().sigil_name_aliases[item.name]

    start = next((i for i, s in enumerate(tts_section) if "AFFIXES" in s or "속성" in s), None)
    if start:
    ...
    for affix_name in affixes:
        ...
        if normalized_name is None:
            normalized_name = ""
        if normalized_name in GameCatalog().sigil_name_aliases:
            normalized_name = GameCatalog().sigil_name_aliases[normalized_name]
...
def _create_base_item_from_tts(tts_item: list[str]) -> Item | None:
    ...
    if "bloodied" in tts_item[1].lower():
        item.seasonal_attribute = SeasonalAttribute.bloodied
    item.is_ancestral = "ancestral" in tts_item[1].lower() or "선조" in tts_item[1]
    ...
    search_string = tts_item[1].lower().replace("ancestral", "").replace("선조", "").replace("bloodied", "").strip()
    ...
    if item.name in GameCatalog().unique_name_aliases:
        item.name = GameCatalog().unique_name_aliases[item.name]
    for line in tts_item:
        ...
        if any(label in line.casefold() for label in item_power_labels):
