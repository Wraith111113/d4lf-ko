...
def mark_greater_affixes_from_tooltip(
    tooltip_image: np.ndarray | None, item: Item, short_separator_match: TemplateMatch | None = None
) -> int:
    """Use visible bullet shapes only when every parsed affix has a matching row."""
    if tooltip_image is None or not item.affixes:
        return 0
    separator = short_separator_match or texture.find_seperator_short(tooltip_image)
    if separator is None:
        return 0
    long_separator = texture.find_seperator_long(tooltip_image, separator, match_index=1 if item.inherent else 0)
    count = len(item.inherent) + len(item.affixes)
    bullets = texture.find_bullets_for_templates(
        tooltip_image,
        separator,
        _AFFIX_BULLET_TEMPLATE_REFS,
        expected_count=count,
        max_y=long_separator.region[1] if long_separator is not None else None,
    )
    ...
    for index, bullet in enumerate(bullets):
        if (
            index < len(item.inherent)
            or bullet.score < BULLET_THRESHOLD
            or not bullet.name.startswith("greater_affix_bullet")
        ):
            continue
        affix = item.affixes[index - len(item.inherent)]
