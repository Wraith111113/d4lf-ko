...
@lru_cache(maxsize=3)
def _english_affix_dict(file_name: str) -> dict[str, str]:
    """Load canonical English labels used by external build-site importers."""
    path = Path(BASE_DIR) / "assets" / "lang" / "enUS" / file_name
    with path.open(encoding="utf-8") as file:
        data = json.load(file)
    if not isinstance(data, dict) or not all(isinstance(key, str) and isinstance(value, str) for key, value in data.items()):
        msg = f"Expected a string mapping in {path}"
        raise ValueError(msg)
    return data


def affix_dict_for_item_type(item_type: ItemType | None) -> dict[str, str]:
    """Return English labels because supported external build sites publish English stats."""
    if item_type == ItemType.HoradricSeal:
        return _english_affix_dict("seals_affixes.json")
    if item_type == ItemType.Charm:
        return _english_affix_dict("charms_affixes.json")
    return _english_affix_dict("affixes.json")
...
def match_set_aware_seal_affix(stat_clean: str, affix_dict: dict[str, str], guessed_set_name: str) -> str | None:
    ...
    set_affixes = {
        key: value for key, value in affix_dict.items() if key.startswith(f"{guessed_set_name}_")
    }
    if not set_affixes:
        return None
