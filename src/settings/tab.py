...
class ConfigTab(ConfigTabMixin, ConfigResetMixin, QWidget):
    def __init__(self, theme_changed_callback: Callable[[], None] | None = None) -> None:
        ...
        self.search_input.setPlaceholderText(f"🔍 {tr('Search settings...')}")
    ...
    def _build_sections(self) -> None:
        ...
        for cat_name in CATEGORY_ORDER:
            ...
            if cat_name == SettingsCategory.HOTKEYS:
                gb_title = tr("Key Bindings")
            elif cat_name == SettingsCategory.ADVANCED:
                gb_title = tr("Technical Settings")
            else:
                gb_title = tr(str(cat_name)).replace("&", "&&")
    ...
    def _add_setting_row(
        ...
        if meta.get(HIDE_FROM_GUI_KEY):
            return
        human_label = tr(meta.get("title") or key.replace("_", " ").title())
        ...
        description = tr(meta.get("description", ""))
