...
class AdvancedOptionsModel(_IniBaseModel):
    ...
    run_filter: str = Field(
        default="f11",
        description="Hotkey to filter character inventory only. If an item matches no profile, it is marked as junk.",
        json_schema_extra={IS_HOTKEY_KEY: "True"},
    )
    run_stash_filter: str = Field(
        default="alt+f11",
        description="Hotkey to filter configured stash tabs only. The stash must be open.",
        title="Run Stash Filter",
        json_schema_extra={IS_HOTKEY_KEY: "True"},
    )
