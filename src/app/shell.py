...
class UnifiedMainWindow(UnifiedWindowLifecycle):
    ...
    def _setup_ui(self) -> None:
        build = f"-{__ko_build__}" if self._config.general.language == "koKR" else ""
        self.setWindowTitle(f"D4LF - Diablo 4 Loot Filter v{__version__}{build}")
        self.setMinimumSize(800, 600)
        self.tabs = QTabWidget()
        self.setCentralWidget(self.tabs)
        self.activity_tab = ActivityLogWidget(parent=self)
        self.console_output = ANSIConsoleWidget()
        self.tabs.addTab(self.activity_tab, tr("Dashboard"))
        self.tabs.addTab(self.console_output, tr("Full Logs"))
    ...
    def _setup_tab_corner_widgets(self) -> None:
        ...
        if sys.platform == "win32":
            self.vision_indicator = QLabel(tr("Vision Mode: STOPPED"))
            self.tts_indicator = QLabel(tr("TTS: Disconnected"))
    ...
    def update_vision_status(self, is_running: bool | None) -> None:
        if is_running is None:
            ...
            return
        self.vision_indicator.setText(tr(f"Vision Mode: {'RUNNING' if is_running else 'STOPPED'}"))
    ...
    def update_tts_status(self, connected: bool | None) -> None:
        if connected is None:
            ...
            return
        self.tts_indicator.setText(tr(f"TTS: {'Connected' if connected else 'Disconnected'}"))
