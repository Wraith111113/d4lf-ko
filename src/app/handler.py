...
from src.loot.equipped_scan import scan_equipped_items
...
class ScriptHandler:
    ...
    def _hotkey_signature(self, config: Settings) -> tuple[str | bool, ...]:
        ...
        return (
            ...
            advanced_options.run_stash_filter,
    ...
    def _register_hotkey(self, hotkey: str, callback: Callable[[], None], check_focus: bool = True) -> None:
        def wrapped_callback() -> None:
            foreground = is_window_foreground(self._win_spec)
            LOGGER.debug("Hotkey received: %s (Diablo foreground: %s)", hotkey, foreground)
            if check_focus and not foreground:
                LOGGER.debug("Hotkey ignored because Diablo IV is not foreground: %s", hotkey)
                return
            try:
                callback()
            except Exception:
                LOGGER.exception("Hotkey callback failed: %s", hotkey)
    ...
    def setup_key_binds(self) -> None:
        ...
        if not advanced_options.vision_mode_only:
            # Filter actions must remain usable while the stash or inventory UI has focus.
            self._register_hotkey(advanced_options.run_filter, lambda: self.filter_items(), check_focus=False)
            self._register_hotkey(
                advanced_options.run_stash_filter, lambda: self.filter_stash_items(), check_focus=False
            )
            self._register_hotkey(
                advanced_options.run_filter_drop, lambda: self.filter_items(no_match_action="drop"), check_focus=False
            )
            self._register_hotkey(
                advanced_options.run_filter_force_refresh,
    ...
    def filter_items(
        ...
        if src.perception.is_connected():
            self._start_or_stop_loot_interaction_thread(run_loot_filter, (force_refresh, no_match_action, "inventory"))
        else:
            LOGGER.warning("Filter hotkey ignored: TTS client is not connected")
            LOGGER.warning(
    ...
    def filter_stash_items(self) -> None:
        if src.perception.is_connected():
            self._start_or_stop_loot_interaction_thread(run_loot_filter, (ItemRefreshType.no_refresh, "junk", "stash"))
        else:
            LOGGER.warning("Stash filter hotkey ignored: TTS client is not connected")

    def move_items_to_inventory(self) -> None:
        self._start_or_stop_loot_interaction_thread(move_items_to_inventory)

    def scan_equipped(self, profile: ProfileModel, publish: Callable[[object], None]) -> None:
        if not src.perception.is_connected():
            msg = "TTS is not connected"
            raise RuntimeError(msg)
        if self.loot_interaction_thread is not None:
            msg = "Another item action is still running"
            raise RuntimeError(msg)

        def run_scan() -> None:
            try:
                scan_equipped_items(profile, publish)
            except Exception as error:
                LOGGER.exception("Equipped gear scan failed")
                publish(error)
            finally:
                publish(None)

        self._start_or_stop_loot_interaction_thread(run_scan)
    ...
    def _wrapper_run_loot_interaction_method(
        ...
        try:
            ...
            if self.vision_mode.running():
                ...
                # Vision callbacks can still be unwinding after stop(). Give the
                # listener and overlay queue a short settling window before the
                # filter starts moving the cursor and reading tooltips.
                time.sleep(0.25)

            loot_interaction_method(*method_args)
        except Exception:
            LOGGER.exception(
                "Loot interaction failed while running %s",
                getattr(loot_interaction_method, "__name__", repr(loot_interaction_method)),
            )
        finally:
            try:
                if self.did_stop_scripts and not self.vision_mode.running():
                    LOGGER.info("Restoring Vision Mode after loot interaction")
                    self.run_vision_mode()
            except Exception:
                LOGGER.exception("Failed to restore Vision Mode after loot interaction")
            finally:
                self.loot_interaction_thread = None
