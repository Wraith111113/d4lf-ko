...
def _generate_affix(
    ...
    return _generate_affix_with_key_context(affix_file, context, context, d4data_dir, language, language)


def _generate_affix_with_key_context(
    affix_file: Path,
    value_context: AffixGenerationContext,
    key_context: AffixGenerationContext,
    d4data_dir: Path,
    value_language: str,
    key_language: str,
) -> tuple[str, str, str] | None:
    ...
    if affix_name.casefold() == "2HStaff_Unique_AF_001_Int_Decrease".casefold():
        return None
    description = None
    string_list_dir = d4data_dir / f"json/{value_language}_Text/meta/StringList"
    ...
    if description is None:
        if affix_data.get("eMagicType") != 0 or not affix_data.get("ptItemAffixAttributes"):
            return None
        description = companion_style_affix_description(affix_data, value_context, d4data_dir, value_language)
    normalised = normalise_affix_description(description)
    if normalised is None:
        return None
    generated_key, value = normalised

    key = generated_key
    if key_language != value_language:
        key_description = None
        key_string_list_dir = d4data_dir / f"json/{key_language}_Text/meta/StringList"
        if is_seal_affix or is_charm_affix:
            key_description = affix_string_description(affix_name, key_string_list_dir)
        if key_description is None:
            key_description = companion_style_affix_description(affix_data, key_context, d4data_dir, key_language)
        key_normalised = normalise_affix_description(key_description)
        if key_normalised is None:
            return None
        key = key_normalised[0]
...
def _affix_generation_context(d4data_dir: Path, language: str) -> AffixGenerationContext:
    core_toc = cast(
...
def generate_affixes(d4data_dir: Path, language: str, output_file: Path | None = None) -> int:
    ...
    context = _affix_generation_context(d4data_dir, language)
    key_language = "enUS" if language != "enUS" and (d4data_dir / "json/enUS_Text/meta/StringList").is_dir() else language
    key_context = context if key_language == language else _affix_generation_context(d4data_dir, key_language)
    ...
    for affix_file in affix_files:
        result = _generate_affix_with_key_context(affix_file, context, key_context, d4data_dir, language, key_language)
        if result is None:
            continue
    ...
    if language != "enUS":
        _fill_missing_canonical_affixes(affix_dict, "affixes")
        _fill_missing_canonical_affixes(seal_dict, "seals_affixes")
        _fill_missing_canonical_affixes(charm_dict, "charms_affixes")
...
def _fill_missing_canonical_affixes(data: dict[str, str], name: str) -> None:
    """Keep every English profile key, falling back to English display text when untranslated."""
    canonical_file = D4LF_BASE_DIR / f"assets/lang/enUS/{name}.json"
    if not canonical_file.exists():
        return
    canonical = cast("dict[str, str]", load_json_file(canonical_file))
