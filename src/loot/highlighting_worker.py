            ...
            while retry_count < 5 and not is_confirmed:
                ...
                if moved_to_new_item:
                    ...
                    if item_descr == self.current_item:
                        if item_descr.affixes and not is_sigil(item_descr.item_type):
                            marked = mark_greater_affixes_from_tooltip(
                                detection.cropped_descr, item_descr, get_separator_match_in_crop(detection)
                            )
                            if marked:
                                LOGGER.info(
                                    "Recognized %s greater affix marker(s) for %s", marked, item_descr.original_name
                                )
