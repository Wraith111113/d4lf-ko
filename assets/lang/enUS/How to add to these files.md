...
Generation currently supports `enUS` and `koKR` when the matching `*_Text/meta/StringList` folders exist in d4data.
For non-English languages, generated files keep D4LF's English canonical keys so profiles and filters remain stable.
Localized values are used for TTS/display matching, and localized names that need to resolve back to canonical keys are
written to `aliases.json`.
