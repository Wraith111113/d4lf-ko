...
def test_move_items_keeps_items_after_capacity_is_reached(monkeypatch) -> None:
    inv = type("Inventory", (), {"hover_item": lambda *_: None})()
    first = ItemSlot((0, 0, 1, 1), (0, 0), is_fav=True)
    second = ItemSlot((1, 0, 1, 1), (1, 0), is_fav=True)
    monkeypatch.setattr("src.automation.loot_mover.Mouse.click", lambda *_: None)
    moved, remaining = _move_items(cast("CharInventory", inv), [first, second], 1, [MoveItemsType.favorites])
    assert moved == 1
