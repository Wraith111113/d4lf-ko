...
from src.item import Affix, FilterResult, Item
...
def test_roll_percent_requires_numeric_range() -> None:
    matcher = FilterMatchingMixin()
    unknown_range = Affix(name="critical_strike_chance", value=5.9)
    assert not matcher._match_item_roll_is_in_percent_range(80, unknown_range)
    assert matcher._match_item_roll_is_in_percent_range(0, unknown_range)

    known_range = Affix(name="critical_strike_chance", value=5.9, min_value=2.5, max_value=6.0)
    assert matcher._match_item_roll_is_in_percent_range(80, known_range)
    assert not matcher._match_item_roll_is_in_percent_range(99, known_range)
