...
def test_affix_dict_for_item_type_uses_context_specific_dict() -> None:
    assert affix_dict_for_item_type(ItemType.Charm)["to_incinerate"] == "to incinerate"
    assert affix_dict_for_item_type(ItemType.HoradricSeal)["damage"] == "damage"
    assert affix_dict_for_item_type(ItemType.Ring)["critical_strike_damage_multiplier"] == (
        "critical strike damage multiplier"
    )
    assert affix_dict_for_item_type(None)["vulnerable_damage_multiplier"] == "vulnerable damage multiplier"


def test_external_english_affixes_resolve_to_canonical_ids_in_korean_mode() -> None:
    affixes = affix_dict_for_item_type(ItemType.Amulet)

    assert closest_match("critical strike damage multiplier", affixes) == "critical_strike_damage_multiplier"
    assert closest_match("vulnerable damage multiplier", affixes) == "vulnerable_damage_multiplier"
    assert closest_match("intelligence", affixes) == "intelligence"
    assert closest_match("critical strike chance", affixes) == "critical_strike_chance"
