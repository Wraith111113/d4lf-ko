...
from src.profiles.validation.normalization import _normalize_tribute_names, _validate_set_name
...
class TestEdgeCases:
    ...
    def test_localized_set_and_tribute_aliases_normalize_to_canonical_names(self, monkeypatch) -> None:
        catalog = GameCatalog()
        monkeypatch.setattr(catalog, "set_list", ["arms_of_testing"])
        monkeypatch.setattr(catalog, "set_name_aliases", {"시험의_무장": "arms_of_testing"})
        monkeypatch.setattr(catalog, "tribute_dict", {"tribute_of_testing": "시험의 공물"})
        monkeypatch.setattr(catalog, "tribute_name_aliases", {"시험의_공물": "tribute_of_testing"})

        assert _validate_set_name("시험의 무장", "set") == "arms_of_testing"
        assert _normalize_tribute_names("시험의_공물") == ["tribute_of_testing"]
