...
def test_affix_search_preserves_saved_selection(qapp, mock_ini_loader) -> None:
    affix = AffixFilterModel(name="critical_strike_chance")
    widget = AffixWidget(affix)
    widget.name_combo.setEditText("unfinished search")
    assert affix.name == "critical_strike_chance"
    widget.name_combo.setEditText("")
    assert affix.name == "critical_strike_chance"
    widget.name_combo.setCurrentText(GameCatalog().affix_dict["critical_strike_damage_multiplier"])
    assert affix.name == "critical_strike_damage_multiplier"


def test_affix_value_and_roll_percent_are_distinct(qapp, mock_ini_loader) -> None:
    affix = AffixFilterModel(name="critical_strike_chance")
    widget = AffixWidget(affix)
    widget.value_edit.setText("5.9")
    assert affix.value == pytest.approx(5.9)
    assert affix.min_percent_of_affix == 0

    widget.mode_combo.setCurrentIndex(widget.mode_combo.findData("Min %"))
    widget.value_edit.setText("80")
    assert affix.value is None
    assert affix.min_percent_of_affix == 80

    widget.mode_combo.setCurrentIndex(widget.mode_combo.findData("Value"))
    assert affix.min_percent_of_affix == 0
    assert affix.value is None


def test_affix_combo_keeps_canonical_id_after_labels_are_reordered(qapp, mock_ini_loader, monkeypatch) -> None:
    choices = {"critical_strike_damage_multiplier": "극대화 피해 계수 x", "to_incinerate": "소각 등급"}
    monkeypatch.setattr(AffixWidget, "get_affix_dict", lambda _self: choices)
    affix = AffixFilterModel(name="critical_strike_damage_multiplier")
    widget = AffixWidget(affix)

    assert widget.name_combo.currentData() == "critical_strike_damage_multiplier"
    widget.name_combo.setCurrentIndex(widget.name_combo.findData("to_incinerate"))
    assert affix.name == "to_incinerate"


def test_duplicate_affix_label_preserves_id(qapp, mock_ini_loader, monkeypatch) -> None:
    choices = {"critical_strike_chance": "Same label", "critical_strike_damage_multiplier": "Same label"}
    monkeypatch.setattr(AffixWidget, "get_affix_dict", lambda _self: choices)
    affix = AffixFilterModel(name="critical_strike_chance")
    widget = AffixWidget(affix)
    widget.update_name("Same label")
    assert affix.name == "critical_strike_chance"
...
def test_item_type_picker_weapon_categories(qapp, mock_ini_loader) -> None:
    item_types = [ItemType.Mace, ItemType.Mace2H, ItemType.Shield, ItemType.Helm]
    picker = ItemTypePicker(None, item_types, [ItemType.Mace])

    assert set(picker.checkboxes) == set(item_types)
    assert list(picker.category_item_types) == [
        "One-handed Weapons",
        "Two-handed Weapons",
        "Off-hand Items",
        "Other Items",
    ]
    picker._set_category_checked(picker.category_item_types["Two-handed Weapons"], checked=True)
    assert set(picker.get_selected_item_types()) == {ItemType.Mace, ItemType.Mace2H}


def test_affixes_tab_hides_legacy_mace_tab(qapp, mock_ini_loader) -> None:
    config = ItemFilterModel(item_type=[ItemType.Axe, ItemType.Mace])
    tab = AffixesTab([DynamicItemFilterModel(root={"Mace": config})])
    tab.load()
    tab.show()
    qapp.processEvents()

    assert tab.item_picker.itemText(0) == "무기"
    assert tab.tab_widget.tabBar().isHidden()
    assert tab.tab_widget.tabText(0) == "Mace"
    tab.close()
