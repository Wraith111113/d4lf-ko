...
def test_catalog_builds_unique_aliases_from_localized_names() -> None:
    aliases = GameCatalog._unique_name_aliases({
        "godslayer_crown": {"localized_name": "신_살해자_왕관", "num_inherents": 0},
        "fists_of_fate": {"num_inherents": 0},
    })

    assert aliases == {"신_살해자_왕관": "godslayer_crown"}


def test_catalog_returns_localized_aspect_and_unique_labels(monkeypatch) -> None:
    catalog = GameCatalog()
    monkeypatch.setattr(catalog, "aspect_display_names", {"accelerating": "가속하는"})
    monkeypatch.setattr(
        catalog, "aspect_unique_dict", {"godslayer_crown": {"localized_name": "신_살해자_왕관", "num_inherents": 0}}
    )

    assert catalog.aspect_label("accelerating") == "가속하는"
    assert catalog.aspect_label("unknown_aspect") == "unknown aspect"
    assert catalog.unique_label("godslayer_crown") == "신 살해자 왕관"
