...
from src.perception.geometry import (
    ...
    mark_greater_affixes_from_tooltip,
)
...
def test_greater_affix_marker_requires_complete_visual_rows(mocker) -> None:
    separator = _make_tm("item_seperator_short_rare", (50, 40))
    long_separator = _make_tm("item_seperator_long_rare", (50, 180))
    image = np.zeros((200, 100, 3), dtype=np.uint8)
    item = Item(affixes=[Affix(name="attack_speed"), Affix(name="vulnerable_damage_multiplier")])
    ...
    bullets = mocker.patch(
        "src.perception.tooltip.texture.find_bullets_for_templates",
        return_value=[_make_tm("affix_bullet_point_1", (10, 60)), _make_tm("greater_affix_bullet_point_1", (10, 80))],
    )

    assert mark_greater_affixes_from_tooltip(image, item) == 1
    assert item.affixes[0].type == AffixType.normal
    assert item.affixes[1].type == AffixType.greater

    item.affixes[1].type = AffixType.normal
    bullets.return_value = [_make_tm("greater_affix_bullet_point_1", (10, 80))]
    assert mark_greater_affixes_from_tooltip(image, item) == 0
    assert item.affixes[1].type == AffixType.normal
