...
def test_korean_affix_generation_keeps_english_internal_keys(tmp_path, monkeypatch) -> None:
    d4data = tmp_path / "d4data"
    affix_dir = d4data / "json/base/meta/Affix"
    affix_dir.mkdir(parents=True)
    (d4data / "json/base/CoreTOC.dat.json").parent.mkdir(parents=True, exist_ok=True)
    (d4data / "json/base/CoreTOC.dat.json").write_text("{}", encoding="utf-8")
    (d4data / "json/GBID.json").write_text("{}", encoding="utf-8")
    for language, text in (("enUS", "Maximum Life"), ("koKR", "최대 생명력")):
        string_dir = d4data / f"json/{language}_Text/meta/StringList"
        string_dir.mkdir(parents=True)
        for name in ("ItemRequirements", "NecromancerArmy", "SkillTags", "UIToolTips"):
            (string_dir / f"{name}.stl.json").write_text('{"arStrings": []}', encoding="utf-8")
        (string_dir / "AttributeDescriptions.stl.json").write_text(
            json.dumps({"arStrings": [{"szLabel": "Life", "szText": text}]}),
            encoding="utf-8",
        )
    (affix_dir / "Life.aff.json").write_text(
        json.dumps({
            "__fileName__": "Life.aff.json",
            "eMagicType": 0,
            "ptItemAffixAttributes": [{"tAttribute": {"__eAttribute_name__": "Life"}}],
        }),
        encoding="utf-8",
    )
    ...
    generate_affixes(d4data, "koKR")

    output = json.loads((output_dir / "affixes.json").read_text(encoding="utf-8"))
    assert output == {"maximum_life": "최대 생명력"}


def test_korean_affix_generation_preserves_missing_english_profile_keys(tmp_path, monkeypatch) -> None:
    d4data = tmp_path / "d4data"
    ...
    output_dir = tmp_path / "assets/lang/koKR"
    canonical_dir = tmp_path / "assets/lang/enUS"
    output_dir.mkdir(parents=True)
    canonical_dir.mkdir(parents=True)
    (canonical_dir / "affixes.json").write_text('{"intelligence": "intelligence"}', encoding="utf-8")
    (canonical_dir / "seals_affixes.json").write_text('{"damage": "damage"}', encoding="utf-8")
    (canonical_dir / "charms_affixes.json").write_text("{}", encoding="utf-8")
    monkeypatch.setattr("src.tools.data_generation.affixes.D4LF_BASE_DIR", tmp_path)

    generate_affixes(d4data, "koKR")

    assert json.loads((output_dir / "affixes.json").read_text(encoding="utf-8"))["intelligence"] == "intelligence"
    assert json.loads((output_dir / "seals_affixes.json").read_text(encoding="utf-8"))["damage"] == "damage"
